...
const aggregatedData = [
  { area: 'District 1', usage: 15420, alerts: 3, efficiency: 78 },
  { area: 'District 2', usage: 18900, alerts: 7, efficiency: 65 },
  { area: 'District 3', usage: 12300, alerts: 1, efficiency: 85 },
  { area: 'District 4', usage: 21500, alerts: 12, efficiency: 58 },
  { area: 'District 5', usage: 16800, alerts: 5, efficiency: 72 },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Monitor system-wide water usage and manage alerts</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium opacity-90">Total Usage Today</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium opacity-90">Connected Meters</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Daily consumption by area</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              ...
              <tbody>
                {aggregatedData.map((district, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">{district.usage.toLocaleString()}L</td>
                    <td className="py-3 px-4">
                      <Badge variant={district.alerts > 5 ? 'destructive' : 'secondary'}>
                    ...
                    <td className="py-3 px-4">
                      <Badge variant={district.efficiency > 70 ? 'default' : 'secondary'}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest automated reports from field devices</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {reportData.map((report) => (
              <div key={report.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-500">Meter: {report.meterCode}</div>
                  <div className="text-xs text-gray-400">{report.timestamp}</div>
                </div>
                <div className="text-right">
                  <div className="font-medium">{report.reading}</div>
                  <Badge variant={report.status === 'high' ? 'destructive' : 'default'}>
                    {report.status}
                  </Badge>
                </div>
              </div>
